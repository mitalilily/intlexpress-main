...
const runAuthBillingSchemaCompatibility = async () => {
  await pool.query(`
  ...
  await pool.query(`ALTER TYPE "invoice_status" ADD VALUE IF NOT EXISTS 'disputed';`)
  await pool.query(`ALTER TYPE "invoice_status" ADD VALUE IF NOT EXISTS 'overdue';`)
...
export const ensureAuthBillingSchemaCompatibility = async () => {
  if (!compatibilityPromise) {
    compatibilityPromise = runAuthBillingSchemaCompatibility().catch((error) => {
